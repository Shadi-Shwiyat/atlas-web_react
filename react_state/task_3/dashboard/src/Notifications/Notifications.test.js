...
test('calls markNotificationAsRead when notificationItem is clicked', () => {
  const markNotificationAsRead = jest.fn(); // Mocking the markNotificationAsRead function
  const listNotifications = [
    { id: 1, type: "default", value: "New course available" }
  ];
  ...
  const wrapper = shallow(
    <Notifications
      ...
      markNotificationAsRead={markNotificationAsRead} // Pass the mock function as a prop
    />
  );

  // Simulate a click on the first NotificationItem
  wrapper.find(NotificationItem).first().simulate('click');

  // Expect the markNotificationAsRead function to be called with the correct argument
